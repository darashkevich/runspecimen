...
6. After the human confirms approval, run sequentially: `preflight` → `run` →
   ...
   refusal.
7. One approved run holds one exclusive lease. Do not fan the same approved
   campaign across subagents or worktrees.
8. RunSpecimen is not an OS sandbox. Recommend a container for untrusted
   payloads.
