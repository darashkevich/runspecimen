...
Stub design for shipping a signed `runspecimen` engine inside the app bundle so
Target A (Mac App Store) does not depend on a user-installed PyPI tool
(guideline **2.4.5(viii)** risk).

**Status:** design + directory scaffold only. No helper binary is built or
bundled in v1. Developer ID (Target B) remains the recommended ship channel.
...
RunSpecimen.app/Contents/
  ...
  Helpers/
    runspecimen              # signed executable entry (or launcher)
    runspecimen-engine/      # optional: embedded Python runtime + package
  Resources/…
```

Entitlement posture (draft):

- App keeps App Sandbox + `user-selected.*` for workspaces.
- Helper uses `com.apple.security.inherit` when spawned as a child, **or** is
  co-signed with an explicit Mach-O identity documented in APP_STORE.md.
...
When helper support is implemented, `CLIService` should resolve in this order:

1. Security-scoped bookmark from Open panel (always valid; user override).
2. Bundled `Contents/Helpers/runspecimen` if present and executable.
3. PATH / PyPI common locations (Developer ID / local debug only).

MAS builds should prefer (1) or (2) and not rely on (3).

## What this stub includes now

- `Helpers/README.md` (this file)
- `Helpers/.gitkeep` so the directory is tracked
- `Helpers/payload/` gitignored — place local build artifacts here during experiments; never commit interpreter tarballs with secrets
...
## Next engineering steps (when unblocked)
...
3. codesign helper + main app with same Team ID; notarize as one bundle (Target B practice run).
4. Wire discovery order in `CLIService`; add Settings toggle “Use bundled engine.”
5. Update App Review notes in APP_STORE.md with helper path + demo instructions.
