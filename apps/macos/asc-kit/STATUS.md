...
| Apple Distribution cert + MAS profile | **Present on this operator Mac** (2026-09-18): identity `Apple Distribution: YAHOR DARASHKEVICH (UN6KF8636A)`; profile `RunSpecimen MAS` (`OSX`, `UN6KF8636A.com.darashkevich.runspecimen`, no `get-task-allow`, no `ProvisionedDevices`). Developer ID Application is **not** a MAS substitute. |
