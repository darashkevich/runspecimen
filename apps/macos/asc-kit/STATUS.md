...
| Store export fail-closed without Apple Distribution | Done — app path derived from `RS_ARCHIVE_PATH` (mismatched `RS_ARCHIVE_APP` refused); real `codesign -dv` of archive binaries; `RS_TEST_CODESIGN_DV_*` harness-only; `test_store_export_gate.sh` negatives prove ad-hoc / bypass / fixtures refused before `xcodebuild -exportArchive` |
